...
import { ModalMap, ModalHeader } from '@/components'
...
export function PlaceConfirmModalContent() {
  ...
  const [center, setCenter] = useState<kakao.maps.Coords>(new kakao.maps.Coords(127.0234346, 37.4979517))
  ...
  return (
    <div className="absolute flex flex-wrap w-1/3 h-1/2 justify-center top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
      ...
      <ModalMap markers={markers} bounds={bounds} center={center} isFetching={isFetching} />
      <ModalButtons />
    </div>
  )
}
